...
const musicOptions = [
    ...
    { value: './music/happybirtday_uia.mp3', label: 'Happy Anniversary (Mèo UIA)' },
    { value: './music/music.mp3', label: 'Happy Anniversary (Instrumental)' },
    { value: './music/postcardnam.mp3', label: 'Happy Anniversary Podcast (Nam)' },
    { value: './music/podcardnu.mp3', label: 'Happy Anniversary Podcast (Nữ)' },
...
function getIdlePreviewMessage() {
    ...
    return label ? `Terpilih: ${label}` : 'Pilih musik lalu klik "Coba dengarkan"';
}

function setMusicPreviewState({ message, isPlaying }) {
    if (musicPreviewButton) {
        musicPreviewButton.textContent = isPlaying ? '⏸ Berhenti mendengarkan' : '▶ Coba dengarkan';
        if (isPlaying) {
...
function handleMusicPreview() {
    ...
    if (!musicSelect || !musicSelect.value) {
        setMusicPreviewState({
            message: 'Silakan pilih musik sebelum mendengarkan',
            isPlaying: false
        });
        return;
    }
    ...
    musicPreviewAudio.play().then(() => {
        setMusicPreviewState({
            message: `Sedang diputar: ${selectedLabel}`,
            isPlaying: true
        });
    }).catch(error => {
        console.error('Gagal memutar contoh musik:', error);
        stopMusicPreview('Gagal memutar contoh. Silakan coba lagi.');
    });
}
...
const colorThemes = {
    pink: {
        ...
        name: 'Merah muda manis'
    },
    blue: {
        matrixColor1: '#87ceeb',
        matrixColor2: '#4169e1',
        sequenceColor: '#1e90ff',
        name: 'Biru laut keren'
    },
    purple: {
        matrixColor1: '#dda0dd',
        matrixColor2: '#9370db',
...
if (index === 0) {
    ...
    title.textContent = t('pageTitle', {num: index + 1});
        ...
        if (page.image) {
            ...
            imagePreview.alt = `Gambar halaman ${index + 1}`;
        } else {
        ...
        if (page.selectedFile) {
            ...
            reader.onload = function (e) {
                ...
                imagePreview.alt = `Gambar baru untuk halaman ${index + 1}`;
            };
        ...
        fileInput.addEventListener('change', function (e) {
            ...
            if (file) {
                ...
                if (page.image && !page.selectedFile) {
                    imagePreview.src = page.image;
                    imagePreview.alt = `Gambar saat ini halaman ${index + 1}`;
                } else {
                    // Hiển thị placeholder
                    const placeholderText = index === 0 ? t('coverPlaceholder') : t('pagePlaceholder', {num: index + 1});
                    imagePreview.style.cssText += `
...
function createPages() {
    ...
    photoUrls = pages.filter(page => page.image && !page.image.includes('theend.jpg')).map(page => page.image);
    ...
    try {
        ...
        if (result.success && result.data) {
            ...
            window.settings = {
                ...
                matrixText: serverSettings.matrixText || 'HAPPYANNIVERSARY',
                matrixColor1: serverSettings.matrixColor1 || '#ffb6c1',
                matrixColor2: serverSettings.matrixColor2 || '#ffc0cb',
                sequence: serverSettings.sequence || 'HAPPY|ANNIVERSARY|TO|YOU|❤',
