...
# Process each file
$files = Get-ChildItem -Path "c:\Users\MML\Documents\projek2\projek2" -Recurse -Include *.js, *.html | Where-Object { $_.FullName -notmatch "node_modules|\.git" }

# Sort replacement keys by length descending to prevent partial replacements
$sortedKeys = $replacements.Keys | Sort-Object Length -Descending

foreach ($file in $files) {
    if ($file.Name -eq "lang.js") { continue } # We already handled lang.js manually
